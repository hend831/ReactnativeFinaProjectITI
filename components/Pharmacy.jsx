...
const Pharmacy = ({ pharmacyInfo, onPressChatPage }) => {
  ...
  const handleaddfriend = async () => {
    ...
    const pharm = pharmacyInfo.id;
    const patient = parseduser[0].id
    

    
    try {
      const response = await axios.post(`${API_URL}/addfriendpharm`, {
        pat_id: patient, friend_pharm_id: pharm 
      });
      ...
      console.log(error);
      alert("pharmacy is already exist in your friends");
    }
  };
    ...
    <View style={styles.container}>
      <TouchableOpacity onPress={openModal} style={styles.pharmacyCard}>
        <View style={styles.profileContainer}>
          <Image source={require('../../New folder (11)/assets/far.png')}
          // {pharmacyInfo.image} 
          style={styles.profileImage} />
        </View>
        <View style={styles.detailsContainer}>
          <Text style={styles.name}>{pharmacyInfo.username}</Text>
